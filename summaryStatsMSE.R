...
if (CI == 1) {
  stats <- cbind(latentTimes, MSE, bias, coverage, CI_size, Per_RMSE)
  colnames(stats) <- c("latentTime", "MSE", "bias", "coverageProb", "CISize", "Per_RMSE")
} else {
  stats <- cbind(latentTimes, MSE, bias, Per_RMSE)
  colnames(stats) <- c("latentTime", "MSE", "bias", "Per_RMSE")
}
